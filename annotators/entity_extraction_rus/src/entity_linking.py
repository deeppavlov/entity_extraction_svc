...
class EntityLinker(Component, Serializable):
    ...
    def load(self) -> None:
        if self.db_format == "sqlite":
            if self.tags_filename:
                ...
                for tag in tags:
                    if tag.lower() != "black":
                        conn = sqlite3.connect(f"{self.load_path}/el_db_rus_{tag.lower()}.db", check_same_thread=False)
                        cur = conn.cursor()
    ...
    def link_entities(self, entity_substr_list: List[str],
        ...
        if entity_substr_list:
            ...
            short_text = False
            if len(re.findall(self.re_tokenizer, text)) <= 3:
                short_text = True
            tm1 = time.time()
            ...
            if self.using_custom_db:
                ...
                entity_tags_dict, init_cand_ent_scores_dict = self.get_cand_ent_wikidata(entity_substr_list,
                    entity_substr_split_list, entity_sent_list, tags_with_probas_list, sentences_list, short_text)
            ...
            for entity_substr, tag, cand_ent_scores in zip(entity_substr_list, substr_tags_list,
                ...
                log.info(f"----- cand_ent_scores_init {cand_ent_scores_init[:2]}")
                ...
                locations = [elem[1][5] for elem in cand_ent_scores]
                for entity_id, location in zip(entity_ids, locations):
                    if location:
                        locations_dict[entity_id] = location
            ...
            if self.use_connections:
                ...
                entity_ids_list, pages_list, entity_tags_list, conf_list, wiki_types_list = self.postprocess_entities(entity_substr_list,
                    ...
                    entities_types_dict, sentences_list, sentences_offsets_list, locations_dict)
    ...
    def get_cand_ent_wikidata(self, entity_substr_list, entity_substr_split_list, entity_sent_list,
                                    tags_with_probas_list, sentences_list, short_text):
        entity_tags_dict = {}
        init_cand_ent_scores_dict = {n: [] for n in range(len(entity_substr_list))}
        already_found = set()
                        ...
                        if not is_already_found:
                            ...
                            if tags_for_search:
                                cand_ent_scores = self.get_cand_ent(entity_substr, entity_substr_split,
                                    tags_for_search, entity_sent, sentences_list, p641_ent, p641_tr, short_text)
                            if cand_ent_scores:
                                ...
                                new_p641_ent, new_p641_tr = self.postprocess_types_for_entity_filter(entity_substr,
                                    entity_sent, tags_for_search, cur_substr_score, cur_types, cur_p641)
                                p641_ent = p641_ent.union(new_p641_ent)
                                p641_tr = p641_tr.union(new_p641_tr)
                                if cur_tag.lower() == "type_of_sport":
                                    p641_tr = {(entity_sent, cur_ent)}
    ...
    def postprocess_entities(self, entity_substr_list, entity_substr_split_list, entity_offsets_list, substr_tags_list,
                                   ...
                                   sentences_list, sentences_offsets_list, locations_dict):
        ...
        for entity_substr, entity_substr_split, entity_offsets, tag, entity_sent, entities_with_conn_scores in \
            ...
            if len(entity_substr_split) >= 4 or tag in {"TYPE_OF_SPORT", "ORG", "CITY", "RIVER", "COUNTY"}:
            ...
            top_entities_with_scores_list.append(top_entities_with_scores)

        tags_in_text_cnts = {}
        for top_entities_with_scores in top_entities_with_scores_list:
            if top_entities_with_scores:
                if top_entities_with_scores[0][-1].lower() in tags_in_text_cnts:
                    tags_in_text_cnts[top_entities_with_scores[0][-1].lower()] += 1
                else:
                    tags_in_text_cnts[top_entities_with_scores[0][-1].lower()] = 1
        main_location = ""
        locations_in_text = {}
        if tags_in_text_cnts.get("city", 0) + tags_in_text_cnts.get("county", 0) > 4:
            for top_entities_with_scores in top_entities_with_scores_list:
                if top_entities_with_scores and top_entities_with_scores[0]:
                    entity_id = top_entities_with_scores[0][0]
                    if locations_dict.get(entity_id, ""):
                        locations = locations_dict[entity_id].split()
                        for location in locations:
                            if location in locations_in_text:
                                locations_in_text[location] += 1
                            else:
                                locations_in_text[location] = 1
            locations_in_text = list(locations_in_text.items())
            locations_in_text = sorted(locations_in_text, key=lambda x: x[1], reverse=True)
            if locations_in_text and locations_in_text[0][1] > 4:
                main_location = locations_in_text[0][0]

        if main_location:
            for j in range(len(top_entities_with_scores_list)):
                if top_entities_with_scores_list[j] and top_entities_with_scores_list[j][0] \
                        and top_entities_with_scores_list[j][0][-1].lower() in {"city", "county"} \
                        and locations_dict.get(top_entities_with_scores_list[j][0][0], "") \
                        and locations_dict.get(top_entities_with_scores_list[j][0][0], "") != main_location:
                    for elem in top_entities_with_scores_list[j]:
                        if main_location in locations_dict.get(elem[0], "").split():
                            top_entities_with_scores_list[j] = [elem]
                            break
    ...
    def get_cand_ent(self, entity_substr, entity_substr_split, tags_for_search, entity_sent, sentences_list,
        ...
        if tags_for_search and tags_for_search[0] == "TYPE_OF_SPORT":
            cur_p641 = []

        tm_st = time.time()
        if self.db_format == "sqlite":
            cand_ent_init = self.find_exact_match_sqlite(entity_substr, tags_for_search,
                                                         {"P641": cur_p641}, short_text)
        else:
    ...
    def make_query_str(self, entity_substr, tags=None, rels_dict=None):
        ...
        loc_extras = ["река", "озеро", "гора", "город", "регион", "холм", "пещера", "пик"]
        if isinstance(entity_substr, str):
            entity_substr = entity_substr.replace("ё", "е").strip()
            if len(entity_substr) > 1:
                make_query_flag = True
            title_str = f"title:{entity_substr}"
        else:
            if all([len(elem) > 0 for elem in entity_substr]):
                entity_substr_title = [f"title:{elem}" for elem in entity_substr]
                if len(entity_substr) == 1:
                    title_str = f"title:{entity_substr[0]}"
                elif len(entity_substr) == 2 and any([ent in loc_extras for ent in entity_substr]):
                    title_str = [f"title:{ent}" for ent in entity_substr if ent not in loc_extras][0]
                elif len(entity_substr_title) == 2:
                    title_str = f"({' OR '.join(entity_substr_title)})"
                elif len(entity_substr_title) > 2:
                    entity_lists = []
                    for i in range(len(entity_substr_title) - 1):
                        entity_lists.append(" AND ".join(entity_substr_title[i:i + 2]))
                    for i in range(len(entity_substr_title) - 2):
                        entity_lists.append(" AND ".join([entity_substr_title[i], entity_substr_title[i + 2]]))
    ...
    def find_exact_match_sqlite(self, entity_substr, tags, rels_dict=None, short_text=None):
        ...
        if tags[0] == "PER" and re.search(r'(\w\.)?\w\.\w+', entity_substr):
            entity_substr = entity_substr.replace(",", "").replace("  ", " ").split(".")[-1]
        else:
        ...
        if entity_substr:
            if self.tags_filename and not self.using_custom_db:
                for tag in tags:
                    ...
                    log.info(f"query_str {query_str} entity_substr {entity_substr} tag {tag}")
                    ...
                    if self.lang=="@ru":
                        entity_substr_normal = self.morph_parse(entity_substr)
                        query_str, make_query_flag = self.make_query_str(entity_substr_normal, None, rels_dict)
                        log.info(f"query_str {query_str} entity_substr {entity_substr_normal} tag {tag}")
                        if tag.lower() != "black" and tag.lower() in self.cursors and make_query_flag:
                            res = self.cursors[tag.lower()].execute(inv_index_query, (query_str,))
                            entities_and_ids = res.fetchall()
                            if entities_and_ids:
                ...
                if not cand_ent_init and (short_text or self.test_mode):
                    for cur_tag in self.cursors:
                        if cur_tag != "black" and cur_tag not in [tag.lower() for tag in tags]:
                            query_str, make_query_flag = self.make_query_str(entity_substr, None, rels_dict)
                            res = self.cursors[cur_tag].execute(inv_index_query, (query_str,))
                            entities_and_ids = res.fetchall()
                            if entities_and_ids:
                                cand_ent_init = self.process_cand_ent(cand_ent_init, entities_and_ids,
                                                                      entity_substr_split, [cur_tag])
            else:
        ...
        if not cand_ent_init and rels_dict:
            if self.tags_filename and not self.using_custom_db:
                for tag in tags:
    ...
    def find_fuzzy_match_sqlite(self, entity_substr_split_raw, tags):
        cand_ent_init = defaultdict(set)
        tags = [tag for tag in tags if tag.lower() != "black"]
        entity_substr_split = []
        for elem in entity_substr_split_raw:
            if self.delete_hyphens:
                elem = elem.replace("-", " ")
                for symb in string.punctuation:
                    elem = elem.replace(symb, "")
            clean = self.morph_parse(elem.replace(".", "").replace(",", "")).replace("ё", "е")
            entity_substr_split.append(clean.strip())
        if tags[0] == "PER" and len(entity_substr_split) == 2:
            entity_substr_split = [entity_substr_split[1]]
        if self.tags_filename and not self.using_custom_db:
            for tag in tags:
                entities_and_ids = []
                query_str, flag = self.make_query_str(entity_substr_split)
                try:
            ...
            query_str, flag = self.make_query_str(entity_substr_split, tags)
            try:
                query = "SELECT * FROM inverted_index WHERE inverted_index MATCH '{}';".format(query_str)
    ...
    def morph_parse(self, word: str):
        normal_word = []
        for w in word.split():
            morph_parse_tok = self.morph.parse(w)[0]
            normal_form = morph_parse_tok.normal_form
            normal_word.append(normal_form)
        return " ".join(normal_word)
    ...
    def calc_substr_score(self, cand_entity_id, cand_entity_title, entity_substr_split, tags, name_or_alias="name"):
        ...
        for ent_tok in entity_substr_split:
            ...
            for label_tok in label_tokens:
                if self.morph_parse(label_tok) == self.morph_parse(ent_tok):
                    found = True
                    break
            if found:
                cnt += 1.0
                ...
                for label_tok in label_tokens:
                    if label_tok[:2] == ent_tok[:2]:
                        fuzz_score = fuzz.ratio(self.morph_parse(label_tok), self.morph_parse(ent_tok))
