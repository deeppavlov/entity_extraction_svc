...
def start_model_server(model_config: Path,
    ...
    # TODO get actual server value from model_config metadata
    server = BaseModelServer(model)
    ...
    async def answer(item: Batch = Body(..., example=model_endpoint_post_example)) -> List:
        ...
        return await loop.run_in_executor(None, server.interact, item.dict())

    @app.post('/probe', include_in_schema=False)
    async def probe(item: Batch) -> List[str]:
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, server.test_interact, item.dict())
...
class BaseModelServer:
    ...
    def test_interact(self, payload: Dict[str, Optional[List]]) -> List[str]:
        model_args = [arg or ["Test string."] for arg in payload.values()]
        try:
            _ = self.model(*model_args)
            return ["Test passed"]
        except Exception as e:
            raise HTTPException(status_code=400, detail=repr(e))


class DreamModelServer(BaseModelServer):
    def interact(self, payload: Dict[str, Optional[List]]) -> List:
        model_args = payload.values()
        dialog_logger.log_in(payload)
        error_msg = None
